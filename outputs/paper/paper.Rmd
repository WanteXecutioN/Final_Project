---
title: "Investigating the impacts of COVID-19 on team wins and social media engagement in the EPL and Manchester United"
author: "Swarnadeep Chattopadhyay" 
thanks: "Code and data are available at: https://github.com/WanteXecutioN/Final_Project."
date: "`r format(Sys.time(), '%d %B %Y')`"
abstract: "The recent developments of the COVID-19 pandemic has had a tremendous impact on the world. Albeit school, work or daily life, there has been changes to the way that we conduct our daily lives. While a lot has changed, the intensity of the major soccer competitions all around the world has not. With teams playing on a regular schedule with advanced COVID protocols, we wanted to assess if teams in the English Premier League have been severely impacted by the change in environment. This report investigates the impact of COVID from the standpoint of team wins and team engagement in the English Premier League(EPL) and uses Manchester United which is a club playing the league to prove the point. Proving this would indicate the effect this pandamic has all over the world in the sports industry.  @citeR is the software used to perform this analysis. "
output:
...
# Introduction
The English Premier League is renowned to be one of the most watched soccer leagues in the world. To date, the league is broadcasted in over 200 countries which spans over 4 billion people (@citeRichards). With such a large market, teams in the EPL are continually able to attract some of the best players in the world. This is pivotal because, not only will the league be attracting more engagement, but the level of competition will be more intense. At the start of 2020, the COVID-19 pandemic had halted a lot of major activities around the world. With the virus spreading quick, the organizers of the EPL halted the premier league football from March, 2020 to June, 2020. After the initial suspension of the games, teams were allowed to play again but behind closed doors meaning no attendance and stricter protocols. We chanced upon an article on ESPN, that essentially highlights the loss of home advantage arising from games with no fans. Pivotally, they pointed out that Liverpool, who were the 2019-2020 season winners, had lost 6 games at home during the COVID era. Prior to that, they were on a 68 games win streak at home (@citeHamilton). We used @citetidyverse, @citegridExtra, @citehrbrthemes, @citefitdistrplus, @citereadxl and @citerms to prove how Covid has affected teams in the EPL both on and off the field. The graphs were printed using @citeggplot2 With the help of @citeknitr, this paper was made possible to print in a pdf format.

The paper looks to compare the effect fans had when describing team's win and as the statistic proves that there is a difference. We can see clearly from the table how Liverpool were champions in 2019-2020 season with 99 points while in the 2020-2021 season, Liverpool finished in 3rd place with 69 points. That shows a 30 point gap. Although we are not looking at Liverpool, they are a perfect example of how Covid has affected their performances without fans. In regards to how Covid has affected Twitter Engagement for Manchester United, the highest total engagement by month has increased by 90% from a number of roughly 5 million to 9.5 million. Although twitter engagement has nothing to do with the team's performance, it is still interesting to notice how fans are finding different ways to support their teams during tough times. As majority of the people were working from home, the high engagement is justified.

The paper goes on to give explanations using graphs and tables in order to provide visuals and prove the negative/positive effects done in the football(soccer) industry by Covid. The paper concludes by answering the 2 key research questions that we look for answers in the next section.
...
In terms of collecting the data, there were numerous websites used to conduct this paper. Firstly, the 2019-2020 season data was generated from @cite19-20. This site also provided the season data for the 2020-2021 season using @cite20-21. Regarding Manchester United, @citeUnited1920 was used to generate their stats for the 2019-2020 season. Looking at the 2020-2021 season, @citeUnited2021 was used. As seen, fbref is a great website that has all the stats regarding the season or even clubs playing in the PL. This made collecting the data easier. In terms of getting the data for the section on twitter engagements, @citeFootball was used. One needs to create an account to be able to access all the statistics needed. Downloading these data sets from the mentioned sites would be enough to conduct this paper.     

# Exploratory Data Analysis
This investigation will consist of 6 data sets and focus on teams in Premier League (in England). To assess the first question, we will be working with data sets 1-2. For reference, the 2019/2020 season will be associated with Pre-Covid period and 2020/2021 season will be associated with COVID period and the average league season has 38 games.


## Team Wins Data Analysis (1st Question)
```{r, echo=FALSE, results='hide'}
## Data from the Premier League in the 2019/2020 and 2020/2021 Season
early <- read.csv("C:/Users/deeps/Desktop/STA304 Final project/1920.csv")
later <- read.csv("C:/Users/deeps/Desktop/STA304 Final project/2021.csv")
summary(early)
summary(later)
```
```{r,echo=FALSE, results='hide'}
...
later %>% mutate(attendance1 = case_when(Attendance < 250 ~ 'Very Few',
                                      Attendance > 250 ~ 'Some')) %>% mutate(prop=W/MP) -> later1
```

On initial observation, both data sets consists of 20 teams and 15 variables. Some key observations from the 2019/2020 season, the average goals scored by a premier league team is: 51.70 or 1.36 goals per game over the course of 38 games. With the 1.36 goals per game, the average number of wins stands at 14.4 which constitutes roughly 38% win rate (not very good). Mean attendance for all 20 teams is: 29796. 

With the 2020/2021 season, the average goals scored by home team stands is: 51.2 goals or 1.35 goal per game over the course of 38 games. With the 1.35 goals per game, the average number of wins stands at 14.85 which constitutes to roughly 39% win rate. Mean attendance in the 2020/2021 season stands at a mere 462.1.
Note here that, the 2020/2021 season was played behind closed doors with some fans allowed when the situation got better.

Initial comparison, looking after 38 games played in both the seasons, there are two observations to point out: 

1) The win rate in the 2020/2021 season has improved by 1% from 38% to 39%.
2) The goals scored per game by a PL has decreased by 0.01 from 1.36 to 1.35.

Roughly 64.5% decrease in attendance has attributed to a 1% increase in team wins but a 0.01 decrease in goals scored per game!

In order to run a logistic regression, the response variable needs to be of value 0 < y < 1. We need to obtain a new response variable consisting of Wins. To do so, we will create new variable: Prop, which is the number of wins over matches played. Given that a team can't possibly win greater than 38 games, we are assured that our response will be less than 1. Another key change was to assign attendance according to: "High", "Medium" and "Low" which we have assigned according to the upper, median, and lower quantile of attendance variable. Same can be said for the 2020/2021, which we will assign "Very few" and "Some" which by its name, represents teams with very few fans and teams with some fans. 

## Twitter Engagement Analysis for Manchester United (2nd Question)

```{r, echo=FALSE, results='hide'}
## Undivided twitter data for Manchester United
twitter_engagement <- read.csv("C:/Users/deeps/Desktop/STA304 Final project/Twitter-Engagement-Pre-PostCovid.csv",stringsAsFactors=FALSE)
...
twitter_postsJulyDec2020 <- read.csv("C:/Users/deeps/Desktop/STA304 Final project/Twitter-EngagementJuly-Dec2020.csv", stringsAsFactors=FALSE)
...
```
The 3rd data set is the twitter engagement data for Manchester United. To clear up the data to account for periods of COVID, we have divided the data set into 3.
...
Within a year of the pandemic, there is a reasonable difference in the total engagements by month. Specifically, the highest total engagement by month has increased significantly by 90%. This statistic does make sense considering that a lot of EPL watcher's are working from home which means that they have more freedom and time to engagement in various social media.
...
## Aanlytic 1: Results from Team Wins affected by COVID
...
Attendance here is a categorical variable that assigning, "High","Medium", "Low" according to the 25%,50% and 75% of variable attendance. Likewise is applied for the 2020/2021 season, where the only difference is that, "Some" and "Very Few" is applied to teams playing with some fans (more than 250 in attendance) and teams playing with very few fans (less than 250) in the stadium.
...
Results from the regression output of the COVID Data shows a compelling result.The current league leaders, Manchester City, would have the highest predicted win percentage of 70%. In retrospect, they had won 71% of their total games for a marginal difference of 1%. Similar to the 2019/2020 season, Manchester City, like Liverpool, would have a 1 more predicted win (over total matches played) than their actual wins to date. To add on, we predicted the number of games that each team would win at the end of the season. Manchester City is predicted to win 27 games over 38 games.Likewise for the COVID season, the attendance does have an impact on the team wins. Teams characterized with some attendance, has 243 more wins than teams very few attendance.

The major difference between the Pre COVID season and the COVID season shows that the league leaders Liverpool had roughly a 31% drop in wins from 32 wins in the 2019-2020 season to 20 wins in the 2020-2021 season. In a notable difference, For a team like Liverpool, the COVID effect with the lack of fans, has been significant. This supports the theory of ESPN as introduced in the introduction.
...
2) The Pythagorean analysis shows major variance in the actual vs predicted and may not accurately 
...
Shows that Chelsea would have had the highest change in predicted wins. But the validity of this model is questioned 
...
## Analytic 2: Twitter Engagement

### Pre Covid Engagement
```{r,echo=FALSE}
posts_2019 <- twitter_engagement %>% filter(Year.Month >= 2019, Year.Month < 2020)
...
PRECOVID <- read.csv("C:/Users/deeps/Desktop/STA304 Final project/wins1920.csv", stringsAsFactors=FALSE)
...
comp <- read.csv("C:/Users/deeps/Desktop/STA304 Final project/comp.csv",stringsAsFactors=FALSE) %>% filter(Comp =="Premier League")
...
### Difference in Average Engagements vs Predicted Engagement
```{r,echo=FALSE}
...
## Visual 2: Change in Engagement vs Change in Actual Win%
```{r,echo=FALSE}
...
3) While there has been increase in engagement, the increased engagement in Twitter does not necessarily 
...
From the results obtained, we assessed that the effect of COVID was significant for some teams in the EPL. Particularly, Liverpool who had a -30 pts difference from the 2019-2020 season.
Namely, the first observation was that the change in win % from 2019-2020 season was 31% decrease compared to the 2020-2021 season for Liverpool. From classifying attendance into "High", "Medium" and "Low", it can be seen that teams classified with medium attendance did much better than teams with high attendance. Trivially, the 'COVID EFFECT' was most significant with Liverpool, who had a 29% drop in expected win percentage between the two seasons.
...
For Future improvements to the model, we must 
...
3) Having a sub section of the data available between the year 2020-2021 where no fans showed up to the stadium.

4) List of players who were not available to play matches due to Covid symptoms who could have played an important part for the team's win. 

This paper shows how Covid played a negative role in some team's performance which is evident. As players were catching the symptoms, they had to be in isolation for 14 days which meant them missing key games. Getting access of that data would have helped understand why Liverpool went from winning the PL to 3rd place in the following season. Doing these will improve the results and provide a better solution to the question of why one team was affected more than the others.    
